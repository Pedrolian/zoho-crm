...
let upsertArray = Array.apply(null, Array(1500)).map((x, i) => {
  return { Account_Name: `Teste Wiki Integracao #${i}`, C_digo_Cliente_GIX: i.toString() };
});

Zoho.upsertRecords("Accounts", upsertArray, ["C_digo_Cliente_GIX"], (error, response, data) => {
  //console.log([error, response, data]);
  console.log(response.success[0]);
}).then(() => {
  console.log(`finished upsert, step 2 agora`);
});
